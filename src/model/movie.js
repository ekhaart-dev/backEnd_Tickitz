...
model.getBy = async ({ page, limit, orderBy, search }) => {
    try {
        ...
        db.query(`SELECT COUNT(id_movie) as "count" FROM public.movies WHERE true ${filterQuery}`).then((v) => {
...
model.save = async ({ banner, name, director, casts, release, duration, synopsis, genre }) => {
...
model.update = async ({ banner, name, director, genre, casts, release, duration, synopsis }, id) => {
    try {
        const pg = await db.connect()
        await pg.query('BEGIN')
